...
json loadBayesSettingsConfig(const std::string& serow_path) {
    ...
    std::cout << "Loading Bayes optimization parameters from: " << bayes_params_file_path
              << std::endl;
...
std::pair<vectord, vectord> readOptimizationParams(const std::string& jsonFilePath,
                                                   ...
                                                   vectord& lower_bounds, vectord& upper_bounds) {
...
int main(int argc, char** argv) {
    ...
    std::pair<vectord, vectord> bounds = readOptimizationParams(
        ...
        params_to_optimize, lower_bounds, upper_bounds);

    BayesOptimizer bayes_optimizer(data_manager, params_to_optimize.size(), settings,
                                   settings_config["ate_params"]["position_weight"],
                                   settings_config["ate_params"]["orientation_weight"],
                                   params_to_optimize);

    bayes_optimizer.setBoundingBox(lower_bounds, upper_bounds);
