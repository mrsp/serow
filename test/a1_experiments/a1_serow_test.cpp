...
constexpr const char* INPUT_FILE = "../a1_experiments/data/indoor/a1_data.h5";
constexpr const char* OUTPUT_FILE = "../a1_experiments/data/indoor/serow_predictions.h5";
...
int main() {
    try {
        ...
        auto imu_timestamp = readHDF5(INPUT_FILE, "imu/timestamp");
        ...
        auto joint_timestamp = readHDF5(INPUT_FILE, "joint_states/timestamp");
        ...
        auto force_timestamp = readHDF5(INPUT_FILE, "feet_force/timestamp");
        ...
        for (size_t i = 0; i < joint_timestamp.size(); ++i) {
            ...
            joints.insert(
                {"FL_hip_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][0]}});
            joints.insert(
                {"FL_thigh_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][1]}});
            joints.insert(
                {"FL_calf_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][2]}});
            joints.insert(
                {"FR_hip_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][3]}});
            joints.insert(
                {"FR_thigh_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][4]}});
            joints.insert(
                {"FR_calf_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][5]}});
            joints.insert(
                {"RL_hip_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][6]}});
            joints.insert(
                {"RL_thigh_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][7]}});
            joints.insert(
                {"RL_calf_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][8]}});
            joints.insert(
                {"RR_hip_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][9]}});
            joints.insert(
                {"RR_thigh_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][10]}});
            joints.insert(
                {"RR_calf_joint",
                serow::JointMeasurement{.timestamp = joint_timestamp[i][0], .position = joint_positions[i][11]}});
            ...
            EstTimestamp.push_back(joint_timestamp[i][0]);
