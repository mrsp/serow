...
class DDPG:
    def __init__(self, actor, critic, params, device="cpu"):
        ...
        self.min_action = torch.tensor(
            params.get("min_action", -1e4 * torch.ones(self.action_dim)),
            device=self.device,
            dtype=torch.float32,
        )
        self.max_action = torch.tensor(
            params.get("max_action", 1e4 * torch.ones(self.action_dim)),
            device=self.device,
            dtype=torch.float32,
        )
        self.num_updates = 0
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            for target_module, module in zip(
                self.actor_target.modules(), self.actor.modules()
            ):
                if isinstance(target_module, torch.nn.BatchNorm1d):
                    target_module.running_mean.mul_(1.0 - self.tau).add_(
                        self.tau * module.running_mean
                    )
                    target_module.running_var.mul_(1.0 - self.tau).add_(
                        self.tau * module.running_var
                    )

            for target_module, module in zip(
                self.critic_target.modules(), self.critic.modules()
            ):
                if isinstance(target_module, torch.nn.BatchNorm1d):
                    target_module.running_mean.mul_(1.0 - self.tau).add_(
                        self.tau * module.running_mean
        ...
        self.logger.log_training_step(
            self.training_step, actor_loss.item(), critic_loss.item(), None, None
        )
