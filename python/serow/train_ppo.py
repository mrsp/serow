...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        log_std = torch.clamp(log_std, min=-20, max=0.5)
        return mean, log_std
...
def train_policy(datasets, contacts_frame, agents, save_policy=True):
    episode_rewards = {}
    collected_steps = {}
    converged = {}
    best_rewards = {}
        ...
        for episode in range(max_episodes):
            ...
            for step, (imu, joints, ft, gt) in enumerate(zip(imu_measurements, 
                ...
                for cf in contacts_frame:
                    ...
                    if not converged[cf] and collected_steps[cf] >= update_steps:
                        agents[cf].train()
                        collected_steps[cf] = 0
...
def evaluate_policy(dataset, contacts_frame, agents):
...
if __name__ == "__main__":
    ...
    if not loaded:
        # Train the policy
        train_policy(train_datasets, contacts_frame, agents, save_policy=True)
        ...
        for cf in contacts_frame:
            ...
            actor = Actor(state_dim, action_dim, max_action, min_action).to(device)
            critic = Critic(state_dim).to(device)
            best_policy[cf] = PPO(actor, critic, params, device=device)

            best_policy[cf].actor.load_state_dict(checkpoint['actor_state_dict'])
            best_policy[cf].critic.load_state_dict(checkpoint['critic_state_dict'])
            print(f"Loaded optimal trained policy for {cf} from 'policy/ppo/trained_policy_{cf}.pth'")
        evaluate_policy(test_dataset, contacts_frame, best_policy)
    else:
        # Just evaluate the loaded policy
        evaluate_policy(test_dataset, contacts_frame, agents)
