...
class Actor(nn.Module):
    ...
    def get_action(self, state, deterministic=False):
        ...
        if deterministic:
            ...
            noise = torch.FloatTensor(self.noise.sample() * self.noise_scale).to(
                self.device
            )
            action = mean + noise
            ...
            for time_step, (imu, joints, ft, gt, next_kin) in enumerate(
                ...
                if collected_steps >= params["n_steps"]:
                    ...
                    if actor_loss is not None and critic_loss is not None:
                        print(
                            f"[Episode {episode + 1}/{max_episodes}] Step [{time_step + 1}/{max_steps}] Policy Loss: {actor_loss}, Value Loss: {critic_loss}"
                        )
                    collected_steps = 0
