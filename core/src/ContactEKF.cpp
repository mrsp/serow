...
void ContactEKF::predict(BaseState& state, const ImuMeasurement& imu,
    ...
    Qc(ng_idx_, ng_idx_) = imu.angular_velocity_cov + angular_velocity_action_cov_gain_ * Eigen::Matrix3d::Identity();
    Qc(na_idx_, na_idx_) = imu.linear_acceleration_cov + linear_acceleration_action_cov_gain_ * Eigen::Matrix3d::Identity();
    Qc(nbg_idx_, nbg_idx_) = imu.angular_velocity_bias_cov + angular_velocity_bias_action_cov_gain_ * Eigen::Matrix3d::Identity();
    Qc(nba_idx_, nba_idx_) =
        imu.linear_acceleration_bias_cov + linear_acceleration_bias_action_cov_gain_ * Eigen::Matrix3d::Identity();
...
void ContactEKF::updateWithContacts(
    ...
    for (const auto& [cf, cp] : contacts_status) {
        ...
        contacts_position_noise.at(cf) = cs * contacts_position_noise.at(cf) +
            ...
            position_cov + contact_position_action_cov_gain_ * Eigen::Matrix3d::Identity();
        ...
        if (!point_feet_ && contacts_orientation_noise.has_value() && orientation_cov.has_value()) {
            contacts_orientation_noise.value().at(cf) =
                ...
                + contact_orientation_action_cov_gain_ * Eigen::Matrix3d::Identity();
        }
    }
...
void ContactEKF::setAction(const Eigen::VectorXd& action) {
    ...
    contact_position_action_cov_gain_ = 1.0 / action(5);
    if (!point_feet_) {
        orientation_action_cov_gain_ = 1.0 / action(6);
        contact_orientation_action_cov_gain_ = 1.0 / action(7);
    }
}
