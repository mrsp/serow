...
class OdometryMeasurementBuffer {
public:
    ...
    /**
     * @brief Get a OdometryMeasurement at the exact timestamp, or interpolate between measurements
     ...
     */
    std::optional<OdometryMeasurement> get(const double timestamp, 
                                              const double max_time_diff = 0.015) const;

private:
    size_t max_size_{1000};
    bool interpolate_covariance_{false};
    std::deque<OdometryMeasurement> measurements_{};

    /**
     * @brief Interpolate between two vectors of OdometryMeasurements at a given timestamp
     ...
     * @return Interpolated OdometryMeasurement
     */
    OdometryMeasurement interpolate(const OdometryMeasurement& m1, 
