...
bool Serow::initialize(const std::string& config_file) {
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        if (!config["base_linear_velocity_covariance"][i].is_number_float()) {
            std::cerr << "Configuration: base_linear_velocity_covariance[" << i
                      << "] must be float \n";
            return false;
        }
    ...
    if (!config["terrain_height_variance"].is_number_float()) {
        std::cerr << "Configuration: terrain_height_variance must be float \n";
        return false;
    }
    params_.terrain_height_variance = config["terrain_height_variance"];
...
void Serow::filter(ImuMeasurement imu, std::map<std::string, JointMeasurement> joints,
    ...
    if (params_.enable_terrain_estimation && !terrain_estimator_ && params_.is_contact_ekf) {
        ...
        terrain_estimator_->terrain_height_variance = params_.terrain_height_variance;
        ...
        for (const auto& [cf, cp] : kin.contacts_status) {
            ...
            if (cs) {
                ...
                if (!terrain_estimator_->update(con_pos_xy, con_pos_z)) {
                    std::cout
                        << "Contact for " << cf
