...
def load_est(mcap_file, topic):
    data_store = {
        ...
        "bias_acc": [], "bias_gyr": []  # Added arrays for biases
    }
...
# Accel Bias
axs3[0].plot(ts_est, est_data["bias_acc"][:, 0], label="Bias Acc X")
axs3[0].plot(ts_est, est_data["bias_acc"][:, 1], label="Bias Acc Y")
axs3[0].plot(ts_est, est_data["bias_acc"][:, 2], label="Bias Acc Z")
axs3[0].set_ylabel("Accel Bias (m/s²)")
axs3[0].grid(True, alpha=0.4)
axs3[0].legend(loc="upper right")

# Gyro Bias
axs3[1].plot(ts_est, est_data["bias_gyr"][:, 0], label="Bias Gyr X")
axs3[1].plot(ts_est, est_data["bias_gyr"][:, 1], label="Bias Gyr Y")
axs3[1].plot(ts_est, est_data["bias_gyr"][:, 2], label="Bias Gyr Z")
axs3[1].set_ylabel("Gyro Bias (rad/s)")
axs3[1].set_xlabel("Time (s)")
axs3[1].grid(True, alpha=0.4)
