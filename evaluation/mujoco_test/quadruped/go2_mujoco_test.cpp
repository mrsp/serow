    ...
    try {
        ...
        for (const auto& msgView : messages) {
            ...
            if (!USE_JOINT_EFFORT_CONTACT_WRENCH) {
                ...
                if (!j_in.contains("feet_forces")) {
                    throw std::runtime_error(
                        "USE_JOINT_EFFORT_CONTACT_WRENCH=false, but input message has no "
                        "feet_forces.");
                }
                ...
                for (const auto& leg : legs) {
                    ...
                    ft.force = Eigen::Vector3d(j_in["feet_forces"][leg]["x"],
            ...
            if (state.has_value()) {
                ...
                // Contact Positions
                j_out["contact_positions"]["FL_foot"] = {{"x", get_contact("FL").x()},
                                                         {"y", get_contact("FL").y()},
                                                         {"z", get_contact("FL").z()}};
                j_out["contact_positions"]["FR_foot"] = {{"x", get_contact("FR").x()},
                                                         {"y", get_contact("FR").y()},
                                                         {"z", get_contact("FR").z()}};
                j_out["contact_positions"]["RL_foot"] = {{"x", get_contact("RL").x()},
                                                         {"y", get_contact("RL").y()},
                                                         {"z", get_contact("RL").z()}};
                j_out["contact_positions"]["RR_foot"] = {{"x", get_contact("RR").x()},
                                                         {"y", get_contact("RR").y()},
                                                         {"z", get_contact("RR").z()}};
