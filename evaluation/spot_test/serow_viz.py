...
def load_gt_data(h5_file):
    with h5py.File(h5_file, "r") as f:
        positions = np.array(f["spot_ground_truth_odometry/_pose/_pose/_position"])
        orientations = np.array(f["spot_ground_truth_odometry/_pose/_pose/_orientation"])
        imu = np.array(f["spot_imu/_linear_acceleration"])

        FL_forces = np.array(f["spot_front_left_foot_force_torque_states/_wrench/_force"])
        FR_forces = np.array(f["spot_front_right_foot_force_torque_states/_wrench/_force"])
        RL_forces = np.array(f["spot_hind_left_foot_force_torque_states/_wrench/_force"])
        RR_forces = np.array(f["spot_hind_right_foot_force_torque_states/_wrench/_force"])
    return (
        positions,
        orientations,
        imu,
...
if __name__ == "__main__":
    ...
    size_diff = gt_pos.shape[0] - timestamps.shape[0]

    FL_forces = FL_forces[size_diff:]
    FR_forces = FR_forces[size_diff:]
    RL_forces = RL_forces[size_diff:]
    RR_forces = RR_forces[size_diff:]
    ...
    # Plotting Ground Truth and Estimated Position (x, y, z)
    figForce, axsForce = plt.subplots(4, 1, figsize=(10, 8), sharex=True)
    figForce.suptitle("Feet forces (z-axis only)")

    axsForce[0].plot(timestamps, FL_forces[:, 2], label="Ground Truth", color="blue")
    axsForce[0].set_ylabel("FL Foot Force")

    axsForce[1].plot(timestamps, FR_forces[:, 2], label="Ground Truth", color="blue")
    axsForce[1].set_ylabel("FR Foot Force")
    
    axsForce[2].plot(timestamps, RL_forces[:, 2], label="Ground Truth", color="blue")
    axsForce[2].set_ylabel("RL Foot Force")

    axsForce[3].plot(timestamps, RR_forces[:, 2], label="Ground Truth", color="blue")
    axsForce[3].set_ylabel("RR Foot Force")

    axsForce[3].set_xlabel("Timestamp")
