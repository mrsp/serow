...
int main(int argc, char** argv) {
    try {
        std::string config_path = "spot.json";  // Default path
        ...
        // Read IMU data
        auto angular_velocity = readHDF5(INPUT_FILE, "spot_imu/_angular_velocity");
        auto linear_acceleration = readHDF5(INPUT_FILE, "spot_imu/_linear_acceleration");
        std::cout << "Opened IMU data from " << INPUT_FILE << std::endl;
        // Read Joint States
        auto joint_positions = readHDF5(INPUT_FILE, "spot_joint_states/_position");
        auto joint_velocities = readHDF5(INPUT_FILE, "spot_joint_states/_velocity");
        std::cout << "Opened Joint States from " << INPUT_FILE << std::endl;
        // Read Feet Forces
        auto feet_force_FL = readHDF5(INPUT_FILE, "spot_front_left_foot_force_torque_states/_wrench/_force");
        auto feet_force_FR = readHDF5(INPUT_FILE, "spot_front_right_foot_force_torque_states/_wrench/_force");
        auto feet_force_RL = readHDF5(INPUT_FILE, "spot_hind_left_foot_force_torque_states/_wrench/_force");
        auto feet_force_RR = readHDF5(INPUT_FILE, "spot_hind_right_foot_force_torque_states/_wrench/_force");

        std::cout << "Opened Feet Forces from " << INPUT_FILE << std::endl;

        // Read Base Pose Ground Truth
        auto base_gt_positions = readHDF5(INPUT_FILE, "spot_ground_truth_odometry/_pose/_pose/_position");
        auto base_gt_orientations = readHDF5(INPUT_FILE, "spot_ground_truth_odometry/_pose/_pose/_orientation");
        std::cout << "Opened Base Pose Ground Truth from " << INPUT_FILE << std::endl;
        // Read Timestamps
        auto imu_timestamps = readHDF5(INPUT_FILE, "spot_imu/timestamp");
        auto joint_timestamps = readHDF5(INPUT_FILE, "spot_joint_states/timestamp");
        auto feet_force_timestamps = readHDF5(INPUT_FILE, "spot_hind_right_foot_force_torque_states/timestamp");
        auto base_gt_timestamp = readHDF5(INPUT_FILE, "spot_ground_truth_odometry/timestamp");
        ...
        for (int i = 0; i < FL_contact_position.size(); ++i) {
            FL_contact_position_x.push_back(FL_contact_position[i].x());
            FL_contact_position_y.push_back(FL_contact_position[i].y());
            FL_contact_position_z.push_back(FL_contact_position[i].z());


            FR_contact_position_x.push_back(FR_contact_position[i].x());
            FR_contact_position_y.push_back(FR_contact_position[i].y());
            FR_contact_position_z.push_back(FR_contact_position[i].z());

            RL_contact_position_x.push_back(RL_contact_position[i].x());
            RL_contact_position_y.push_back(RL_contact_position[i].y());
            RL_contact_position_z.push_back(RL_contact_position[i].z());


            RR_contact_position_x.push_back(RR_contact_position[i].x());
            RR_contact_position_y.push_back(RR_contact_position[i].y());
            RR_contact_position_z.push_back(RR_contact_position[i].z());
        }
